...
# 支持两种目录结构：agent 隔离和根目录
MEMORY_DIR="$WORKSPACE/memory"
AGENT_MEMORY_DIR="$WORKSPACE/memory/$AGENT_ID"

# 优先使用 agent 隔离目录，如果不存在则使用根目录
if [ -d "$AGENT_MEMORY_DIR" ]; then
    SCAN_DIR="$AGENT_MEMORY_DIR"
    echo "✅ 使用 agent 隔离目录：$SCAN_DIR"
else
    SCAN_DIR="$MEMORY_DIR"
    echo "ℹ️  使用根目录：$SCAN_DIR"
fi
...
# 获取最近 3 天的记忆文件
TEMP_FILE=$(mktemp)
for f in "$SCAN_DIR"/*.md "$SCAN_DIR"/**/*.md; do
  if [ -f "$f" ]; then
...
# 保存候选列表供后续处理（使用 agent 隔离目录）
CANDIDATES_FILE="$WORKSPACE/data/$AGENT_ID/candidate_entities.txt"
...
| 指标 | 数值 |
|------|------|
| 扫描文件数 | $(ls "$SCAN_DIR"/*.md 2>/dev/null | wc -l | tr -d ' ') |
...
echo ""
echo "统计结果:"
echo "  扫描文件：$(ls "$SCAN_DIR"/*.md 2>/dev/null | wc -l | tr -d ' ') 个"
